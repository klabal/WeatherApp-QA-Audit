...
| 4 | Security: Spec. Chars | Medium | 1. Enter `ĄĖĘ ; #$%^&` in login. | System handles/sanitizes input. | **Fail** | Generic required field error; no sanitization check.  ![Screenshot.](./screenshots/tc-2.png) |
...
| 7 | "Remember Me" | High | 1. Check for "Remember Me" option. | Feature should be available in UI. | **Fail** | Feature is missing from the login form.  ![Screenshot.](./screenshots/tc-3.png) |
...
| 11 | Case Sensitivity | Medium | 1. Search `kaunas` vs `KAUNAS`. | Consistent results regardless of casing. | **Pass** | Case sensitivity handled correctly.  ![Screenshot.](./screenshots/tc-4-1.png)  ![Screenshot.](./screenshots/tc-4-2.png) |
...
| 13 | Result Filtering | Medium | 1. Use the filter field on results. | Filter by Country/City name. | **Fail** | Filter only works by text; no criteria selection.  ![Screenshot.](./screenshots/tc-5.png) |
...
| 31 | Geo (Allowed) | Critical | 1. Grant location permission. | Detects city name automatically. | **Hold** | Permission asks, but label stays blank. ![Screenshot.](./screenshots/tc-geo.png) |
