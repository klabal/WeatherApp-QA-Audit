...
| M6 | Pull-to-Refresh | Medium | 1. Navigate to the main dashboard. 2. Swipe down from the top of the screen. | The app triggers a data refresh or page reload. | **Pass/Fail** |
| M7 | Chart Responsiveness | High | 1. Open the 24-hour hourly temperature chart. 2. Attempt to scroll horizontally. | The chart is fully accessible, scales to width, or allows smooth horizontal scrolling without breaking the layout. | **Pass/Fail** |
| M8 | Visual Fidelity (DPI) | Low | 1. Inspect weather icons and UI elements on the high-resolution Pixel 6 screen. | Icons and assets appear sharp and clear (no pixelation or blurriness). | **Pass/Fail** |
